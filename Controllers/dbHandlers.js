...
class dbHandlers {
    ...
    async addSecret(userId, secret) {
        await pool.query(`INSERT INTO secrets (user_id, secret) VALUES ($1, $2);`, [userId, secret])
    }
    async findSecretBy(userId) {
        return await pool.query(`SELECT secret FROM secrets WHERE user_id = $1;`, [userId])
    }
    async findAddressByID(userId) {
        return await pool.query(`SELECT address FROM eth_keys WHERE user_id = $1;`, [userId])
    }
    ...
    async findPrivateKeyByUserId(userId) {
